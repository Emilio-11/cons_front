...
export default function ReportesPage() {
    ...
    async function buscar() {
        ...
        setGrupoAbierto(null);
        setReportesGrupo([]);
        setEstadoMasivoSeleccionado('');
        setMeta({ total: 0, totalPages: 1, page: filtros.page || 1 });
    ...
    async function cargarGrupo(dataGrupo: any) {
        ...
        if (filtros.agrupar === 'concesionaria' || filtros.agrupar === 'ambos') {
            if (dataGrupo.r_concesionaria) params.set("concesionaria", dataGrupo.r_concesionaria);
        }
        if (filtros.agrupar === 'tipo' || filtros.agrupar === 'ambos') {
            if (dataGrupo.r_tipoReporte) params.set("tipoReporte", dataGrupo.r_tipoReporte);
        }

        params.set("agrupar", "false");
    ...
    return (
        <div className="filtro">
            ...
            {hasSearched && meta.total > 0 && (
                <div className="paginacion" style={{ display: 'flex', gap: '15px', justifyContent: 'center', marginTop: '10px' }}>
                    <button className="btn-paginacion" disabled={meta.page <= 1} onClick={() => setFiltros(prev => ({ ...prev, page: prev.page! - 1 }))}>Anterior</button>
                    <span>Página {meta.page} de {meta.totalPages}</span>
                    <button className="btn-paginacion" disabled={meta.page >= meta.totalPages} onClick={() => setFiltros(prev => ({ ...prev, page: prev.page! + 1 }))}>Siguiente</button>
                </div>
            )}
                                ...
                                {isOpen && (
                                    <div style={{ marginTop: "5px", marginLeft: "20px", padding: "10px", backgroundColor: "#f9fafb", borderLeft: "2px solid #cbd5e1" }}>
                                        ...
                                        <div style={{ padding: '15px', background: '#e0f2fe', borderRadius: '8px', marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px', border: '1px solid #bae6fd' }}>
                                            ...
                                            <select value={estadoMasivoSeleccionado} onChange={(e) => setEstadoMasivoSeleccionado(Number(e.target.value))} style={{ padding: '5px', borderRadius: '4px', border: '1px solid #ccc' }}>
                                                <option value="">Seleccionar nuevo estado...</option>
                                                {estados.map(s => <option key={s.id_Estado} value={s.id_Estado}>{s.estado}</option>)}
                                            </select>
                                            <button className="btn" style={{ backgroundColor: '#0284c7', color: 'white', padding: '5px 15px', borderRadius: '4px', border: 'none', cursor: 'pointer' }} onClick={actualizarEstadoMasivo}>
                                        ...
                                        {reportesGrupo.map((rep, idx) => (
                                            <div key={idx} className="card" onClick={() => setDetalle(rep)} style={{ marginBottom: "10px", padding: "10px", backgroundColor: "white", cursor: "pointer", boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
                                                ...
                                                <p style={{ fontSize: "0.9rem", color: "#4b5563", marginTop: "5px" }}>Descripción: {rep.descripcion?.slice(0, 80)}</p>
                                                <p style={{ fontSize: "0.9rem", color: "#4b5563", marginTop: "2px" }}>Tipo de reporte: {rep.tipoReporte?.tipoReporte?.slice(0, 80)}</p>
                                                <p style={{ fontSize: "0.9rem", color: "#4b5563", marginTop: "2px" }}>Usuario: {rep.usuario?.correo_electronico?.slice(0, 80)}</p>
                                                <p style={{ fontSize: "0.9rem", color: "#4b5563", marginTop: "2px" }}>
                                                Imagen:{" "}
                                                {rep.imagen ? (
                                                    <a
                                                        href={rep.imagen.replace("&export=download", "")}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        style={{ color: "#2563eb", textDecoration: "underline" }}
                                                    >
                                                        {rep.imagen.replace("&export=download", "")}
                                                    </a>
                                                ) : (
                                                    <>No hay imagen</>
                                                )}
                                ...
                                <div className="card resultado">
                                    ...
                                    <div style={{ backgroundColor: '#f3f4f6', padding: '15px', borderRadius: '8px', marginBottom: '20px', border: '1px solid #ddd' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '10px' }}>
                                            <select defaultValue={detalle.estado?.id_Estado || estados.find(e => e.estado === detalle.estado)?.id_Estado} onChange={(e) => actualizarEstadoIndividual(detalle.id_Reporte, Number(e.target.value))} style={{ flex: 1, padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }}>
                                                {estados.map(s => <option key={s.id_Estado} value={s.id_Estado}>{s.estado}</option>)}
                                            </select>
                                        </div>
